...
class Logger {
  ...
  writeLog(content: string): void {
    ...
    vscode.workspace.openTextDocument(this.logFilePath).then(
      (doc) => {
        ...
        vscode.window.showTextDocument(doc, { preview: false }).then(
            ...
            editor
              ...
              .then(
                (applied) => {
                  if (applied) {
                    console.log(
                      `[DEBUG] Log written to: ${this.logFilePath} (prepended)`
                    ); // Log success
                  } else {
                    console.error(
                      "[ERROR] Failed to apply editor edit when writing log entry."
                    ); // Log failure
                ...
                (editErr: unknown) => {
                  ...
                  const errorMessage =
                    editErr instanceof Error
                      ...
                      : String(editErr);
                  console.error(
                    `[ERROR] Failed to edit document: ${errorMessage}`
                  );
                  vscode.window.showErrorMessage(
                    `Failed to edit document: ${errorMessage}`
                  );
                }
              );
          },
          (showErr: unknown) => {
            ...
            console.warn(
              `[WARNING] Unable to show log file in editor: ${errorMessage}`
            );
            ...
            try {
              ...
              fs.writeFileSync(
                ...
                "utf-8"
              ); // Prepend the log entry
              console.log(
                `[DEBUG] Log written to: ${this.logFilePath} (fs fallback, prepended)`
              ); // Log success
              ...
              const fsErrorMessage =
                fsErr instanceof Error ? fsErr.message : "Unknown error";
              console.error(
                `[ERROR] Failed to write to log file: ${fsErrorMessage}`
              );
              vscode.window.showErrorMessage(
                `[ERROR] Failed to write to log file: ${fsErrorMessage}`
              );
      ...
      (openErr: unknown) => {
        ...
        vscode.window.showWarningMessage(
          `[WARNING] Unable to open log file: ${errorMessage}`
        );
      }
    );
  }
  ...
  private openLogFile(): void {
    void vscode.workspace.openTextDocument(this.logFilePath).then(
      ...
      (error) => {
        console.warn(
          `[WARNING] Unable to open log file: ${error?.message || error}`
        ); // Log warning
        vscode.window.showWarningMessage(
          `[WARNING] Unable to open log file: ${error?.message || error}`
        );
...
class CopilotInteractionHandler {
  ...
  private handleDocumentChange(event: vscode.TextDocumentChangeEvent): void {
    ...
    if (
      ...
      // Check if the event is for the active editor
      return; // Exit if not
    }

    const deltaText = event.contentChanges
      .map((change) => change.text)
      .join(""); // Get the changed text
    if (!deltaText) {
      // Check if there is any changed text
      return; // Exit if not
    }

    if (!/copilot/i.test(deltaText)) {
      // Check if the changed text contains "copilot"
      return; // Exit if not
    }
...
class ChatSessionScanner {
  ...
  scanAndLog(): void {
    ...
    if (!appData) {
      // Consolidate warning messages into a single variable for reuse
      const message =
    ...
    if (!fs.existsSync(storageRoot)) {
      const message = `[WARNING] VS Code workspaceStorage not found at ${storageRoot}`;
      console.warn(message);
      vscode.window.showWarningMessage(message);
      return;
    }
    ...
    try {
          ...
          try {
            ...
            if (parsed?.messages && Array.isArray(parsed.messages)) {
              messages = parsed.messages
                .map((m: any) => m.text || m.content || String(m))
                .filter(Boolean);
            ...
            let resolvedPath =
              parsed?.workspace?.folder || parsed?.config?.workspacePath || "";
            if (!resolvedPath) {
            ...
            const message = `[WARNING] Failed to read/parse chat session ${chatFilePath}: ${
      ...
      const message = `[WARNING] Error scanning workspaceStorage: ${
        err instanceof Error ? err.message : err
      }`;
      console.warn(message);
      vscode.window.showWarningMessage(message);
...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (!vscode.workspace.isTrusted) {
    vscode.window.showErrorMessage(
      "Workspace is not trusted. Please trust it to enable Copilot logging."
    );
    return;
  }
  ...
  if (!workspaceFolder) {
    vscode.window.showErrorMessage(
      "Open a workspace folder to enable Copilot logging."
    );
    return;
  }
